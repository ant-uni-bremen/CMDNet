...
for snr_i in range(-6, 20, 1):  # range(-6, 20)
    ...
    for ii in range(0, 20):
        fn = os.path.join(path, 'BER_SD_' + mod +
                          '_{}_{}'.format(Nt, Nr) + fn_ext + str(ii) + '.mat')
        if os.path.isfile(fn):
            print(fn)
            one_file = 1
            load_data = sio.loadmat(
                fn)['BER'][0, 0][['EbN0', 'SNR', 'SD', 'Nit', 'Nerr', 'Nb']]  # , 'BitAnz']]
    ...
    if one_file:
        ...
        print('EbN0: {}dB, Nerr: {}, Nit: {}, BER: {:.2e}'.format(
            ebn0[0][0], np.sum(Nerr), np.sum(np.array(Nit).astype('uint64')), ber_tot[-1]))
...
pathfile = os.path.join(path, 'RES_SD_' + mod +
                        '_{}_{}'.format(Nt, Nr) + rho_ext)

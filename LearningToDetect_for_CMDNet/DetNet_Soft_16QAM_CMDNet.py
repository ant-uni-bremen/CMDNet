...
def validate2(y, SNR, H, K, N):
    ...
    for i in range(np.power(2, K)):
        ...
        for j in range(np.power(2, K)):
            ...
            for ii in range(K):
                if x[ii] == 3:
                    final_probs_three[ii] = final_probs_three[ii] + \
                ...
                if x[ii] == -3:
                    final_probs_minus_three[ii] = final_probs_minus_three[ii] + (
                        (1.0 / np.power(2, K)) * prob)
                    sum_minus_three = sum_minus_three + 1
    for ii in range(K):
        norm = final_probs_one[ii] + final_probs_minus_one[ii] + \
            final_probs_minus_three[ii] + final_probs_three[ii]
...
K2 = 64
N2 = 64
K = int(K2 / 2)                         # default: 4, Nt * 2, HD: 15
N = int(N2 / 2)                         # default: 8, Nr * 2, HD: 30
ebn0db_low = 7                          # default: 1 vs. 10 (CMD)
ebn0db_high = 30                        # default: 8 vs. 33 (CMD)
...
train_iter = 80000                      # default: 80000, HD: 200000
train_iter_no_noise = 10000             # default: 10000, HD: 5000 vs. 0 (CMD)
train_batch_size = 5000                 # default: 5000
test_iter = 50                          # default: 50
test_batch_size = 1000                  # default: 500
LOG_LOSS = 1
res_alpha = 0.9
num_snr = 42                            # default: 6
snrdb_low_test = -6 + 6                 # default: 8.0
snrdb_high_test = 36 + 6                # SNR, default: 13.0
# mydefault: np.array([-3, -1, 3, 1])
symbols = np.array([-3, -1, 1, 3])
normalize_dist = 1

# hard decision parameters
# K = 15
# N = 25
# snrdb_low = 7.0
# snrdb_high = 14.0
...
def generate_data(B, K, N, snr_low, snr_high, noise, compl=1, rho=0, Phi_R12=0):
...
HY = tf.placeholder(tf.float32, shape=[None, 2*K], name="HY")
X = tf.placeholder(tf.float32, shape=[None, 2*K], name="X")
X_IND = tf.placeholder(tf.float32, shape=[None, K, 16], name="X_IND")
HH = tf.placeholder(tf.float32, shape=[None, 2*K, 2*K], name="HH")
...
S4 = []
S4.append(tf.zeros([batch_size, K, 4, 2], name="Out"))
V = []
...
for i in range(1, L):
    ...
    Z1.append(S1[-1] - (delta[(i-1) * 2]*HY) +
              (delta[(i-1) * 2 + 1]*temp1[-1]))
    ...
    S2[i] = tf.clip_by_value(S2[i], 0, 1, name="S2")
    ...
    S4_temp1 = tf.concat([temp_0 + temp_1 + temp_2 + temp_3, temp_4 + temp_5 + temp_6 +
                         temp_7, temp_8 + temp_9 + temp_10 + temp_11, temp_12 + temp_13 + temp_14 + temp_15], 2)
    S4_temp2 = tf.concat([temp_0 + temp_4 + temp_8 + temp_12, temp_1 + temp_5 + temp_9 +
                         temp_13, temp_2 + temp_6 + temp_10 + temp_14, temp_3 + temp_7 + temp_11 + temp_15], 2)
    S4_temp1 = tf.expand_dims(S4_temp1, 3)
    S4_temp2 = tf.expand_dims(S4_temp2, 3)

    S4_temp3 = tf.concat([S4_temp1, S4_temp2], 3, name="Out")
...
for i in range(train_iter_no_noise):  # num of train iter
    batch_Y, batch_H, batch_HY, batch_HH, batch_X, SNR1, x_ind = generate_data(
...
for j in range(num_snr):
    ...
    for jj in range(test_iter):
        if jj % 100 == 0:
            ...
            print(jj)
        batch_Y, batch_H, batch_HY, batch_HH, batch_X, SNR1, BATCH_X_IND = generate_data(
        ...
        last_layer = np.array(
            sess.run(S4[-1], {HY: batch_HY, HH: batch_HH, X: batch_X, X_IND: BATCH_X_IND}))

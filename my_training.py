...
eps: Small constant to avoid numerical problems, e.g., 1e-12, since x=0, then NaN!
    ...
    out = x / tf.math.sqrt(tf.reduce_mean(x ** 2 + eps,
...
class training_history():
    ...
    def printh(self):
        ...
        '''
        print_str = "Epoch: {}, Train Loss: {:.6f}, Val Loss: {:.6f}, Time: {:04.2f}s, Tot. time: ".format(
            self.epoch[-1], self.train_loss[-1], self.val_loss[-1], self.train_time[-1]) + print_time(self.total_time)
        print(print_str)
        return print_str
